...
import type { FilterWithPaginationClickAchievementRequest } from "@/features/aopAchievement/models/AopAchievementReportModel";
...
export const AchievementBookingReport: React.FC<Props> = ({ filterType, fromDate, toDate, channelPartnerId, tabName, columnKey }) => {
    ...
    const loadBookingData = useCallback(async (page: number = pagination.currentPage) => {
        ...
        await runApiWithLoader(
            ...
            async () => {
                ...
                const params: FilterWithPaginationClickAchievementRequest = {
                    ...
                    ChannelPartnerId: Number(channelPartnerId) || 0,
                ...
                const response = await aopAchievementReportService.apiCallPullBookingReport(params);
    ...
    const handleExportBooking = async (exportType: 'Excel' | 'PDF') => {
        await runApiWithLoader(
            setIsLoading,
            setLoadingMessage,
            async () => {
                ...
                handleExportFile(response, exportType, 'Achievement By Booking', addToast);
    ...
    const BookingForTable = useMemo(() => bookingList, [bookingList]);

    const requiredBookingColumnKeys: string[] = ['EmployeeName'];
    ...
    const [selectedBookingColumnKeys, setSelectedBookingColumnKeys] = useState<string[]>(() => {
        try {
            ...
            const saved = LocalStorageHelper.getAchievementByChannelPartnerBookingTableColumns?.();
    ...
    useEffect(() => {
        setSelectedBookingColumnKeys(prev => Array.from(new Set([...prev, ...requiredBookingColumnKeys])).filter(k => allBookingColumnKeys.includes(k)));
    ...
    return (
        <div>
            ...
            <TableActionToolbar
                ...
                // EXPORT
                isShowExportButton={canExport && BookingForTable.length > 0}
            ...
            <CustomizeColumnsModal
                ...
                onApply={keys => {
                    ...
                    try {
                        LocalStorageHelper.storeAchievementByChannelPartnerBookingTableColumns?.(
...
export default AchievementBookingReport;
