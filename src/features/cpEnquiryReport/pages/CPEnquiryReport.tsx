...
import type { ChannelPartnerList, FilterWithPaginationCPEnquiryReportRequest } from "@/features/cpEnquiryReport/models/CPEnquiryReportModel";
import { cpEnquiryReportService } from "@/features/cpEnquiryReport/services/CPEnquiryReportService";
...
const CPEnquiryReport: React.FC = () => {
    ...
    const fetchCPEnquiryReport = async (page: number = pagination.currentPage) => {
        ...
        await runApiWithLoader(
            ...
            setLoadingMessage,
            async () => {
                const params: FilterWithPaginationCPEnquiryReportRequest = {
                    PageNumber: page,
                    PageSize: pagination.pageSize,
                    ProjectId: projectId,
                    ChannelPartnerName: channelPartnerNameSearch ?? '',
                    Stage: '',
                ...
                const response = await cpEnquiryReportService.apiCallPullCPEnquiryReport(params);
    ...
    const dynamicHeaders = useMemo<string[]>(() => {
        ...
        if (reportType?.toUpperCase() === "DATE") {
            ...
            channelPartnerList.forEach(emp =>
                emp.ChannelPartnerEnquiryStagesData?.forEach(item => {
                    if (item.Date)
    ...
    }, [channelPartnerList, reportType]);

    const tableData = useMemo<PivotEnquiryRow[]>(() => {

        const rows: PivotEnquiryRow[] = [];

        channelPartnerList.forEach((channelPartner) => {
            ...
            MASTER_DATA.finalStage.forEach((stage, index) => {
                ...
                channelPartner.ChannelPartnerEnquiryStagesData?.forEach(item => {
    ...
    }, [channelPartnerList, dynamicHeaders, reportType]);
...
<DatePickerInput
                ...
                <div className="pt-6">
                    <Input
                        ...
                        value={channelPartnerNameSearch}
                        onChange={(e) => setChannelPartnerNameSearch(e.target.value)}
                    /></div>
                ...
                {canSearch && (
                    <div className="pt-6">
                        <Button color="blue" size="md" onClick={() => fetchCPEnquiryReport(1)}>
                            Search
                        </Button>
                    </div>
                )}
...
export default CPEnquiryReport;