...
import type { FilterWithPaginationPaymentLedgerCrm, PaymentLedgerCrmModelData, AddUpdatePaymentLedgerCrm, DeletePaymentLedgerCrmRequest, } from "@/features/crmPayTrack/models/PaymentLedgerCrmModel";
import { useState, useEffect, useMemo, useCallback } from "react";
import { useProject } from "@/features/projectMaster/context/ProjectContext";
import { runApiWithLoader } from "@/core/utils";
import { paymentLedgerCrmService } from "@/features/crmPayTrack/services/PaymentLedgerCrmService";
...
export const PaymentLedgerCrm: React.FC = () => {
    ...
    const [editingPaymentLedgerData, setEditingPaymentLedgerData] = useState<PaymentLedgerCrmModelData | null>(null);
    const [isAddUpdateModalOpen, setIsAddUpdateModalOpen] = useState(false);

    const [isConfirmationDialogBoxOpen, setIsConfirmationDialogBoxOpen] = useState(false)
    const [deletePaymentLedgerCrmData, setDeletePaymentLedgerCrmData] = useState<PaymentLedgerCrmModelData | null>(null);
    ...
    const handleEditPaymentLedger = useCallback((row: PaymentLedgerCrmModelData) => {
    ...
    const handleConfirmationDialogBoxOpen = useCallback((row: PaymentLedgerCrmModelData) => {
    ...
    const pushAddPaymentLedgerData = (): FormData => {
        ...
        fd.append('BookingOtherChargesId', formData.BookingOtherChargesId?.toString() || '');
    ...
    const handleAddPaymentLedger = async (e: React.FormEvent) => {
        ...
        await runApiWithLoader(
            ...
            async () => {
                ...
                const response = await paymentLedgerCrmService.apiCallAddUpdatePaymentLedgerCrm(payload);
                if (E.isRight(response)) {

                    setIsAddUpdateModalOpen(false);

                    const isAdd = formData.PayTrackPaymentLedgerId === 0;

                    if (isAdd) {
                        const newRecord = response.right.Data[0] as PaymentLedgerCrmModelData

                        setPaymentLedgerCrmList(prevData => [newRecord, ...prevData]);


                        addToast({ type: 'success', title: response.right.SuccessMessage[0] })

                    } else {
                        const updatedRecord = response.right.Data[0] as PaymentLedgerCrmModelData;

                        setPaymentLedgerCrmList(prevData =>
                            prevData.map(item =>
                                item.PayTrackPaymentLedgerId === formData.PayTrackPaymentLedgerId
                                    ? updatedRecord
                                    : item
                            )
                        )
    ...
    const handleDeletePaymentLedgerCrm = async () => {
        ...
        await runApiWithLoader(
            ...
            setLoadingMessage,
            async () => {
                const params: DeletePaymentLedgerCrmRequest = {
                ...
                const response = await paymentLedgerCrmService.apiCallDeletePaymentLedgerCrm(params);

                if (E.isRight(response)) {


                    await loadPaymentLedgerCrm();
    ...
    return (
        <div>
            ...
            <DataTableExpandable
                ...
                recordsPerPage={20}
                expandable={{
                    keyField: 'PayTrackPaymentLedgerId',
                            ...
                            <div className="bg-gray-50 rounded-xl p-4">
                                ...
                                <div className="flex justify-between items-center">
                                    ...
                                    <div className="text-sm text-gray-700">
                                        <FieldItem label="Amount" value={`₹ ${row.ReceivedAmount || 0}`} isRow />
                                ...
                                <div className="grid grid-cols-3 gap-6 text-sm pt-5">
                                    ...
                                    <div className="space-y-3">
                                        ...
                                        <FieldItem label="Transaction / Cheque / Demand Draft No" urls={row.TransactionChequeDemandDraftURL} value={row.TransactionChequeDemandDraftNumber || '-'} />

                                        <FieldItem label="Transaction / Cheque / Demand Draft Date" value={formatDate_dd_MonthName_yy(row.TransactionChequeDemandDraftDate) || '-'} />
                                    </div>
                                    ...
                                    <div className="space-y-3">
                                        ...
                                        <FieldItem
                                            label="Created Date"
                                            value={formatDate_dd_MonthName_yy_hh_mm(row?.CreatedDate ?? '-')}
                                        />
                                        <FieldItem label="Modified By" value={row?.ModifiedBy ?? '-'} />
                                        <FieldItem
                                            label="Modified Date"
                                            value={formatDate_dd_MonthName_yy_hh_mm(row?.ModifiedDate ?? '-')}
                                        />
                                    </div>
                                ...
                                <>
                                    ...
                                    <div>
                                        <Input
                                            ...
                                            value={projectWithBankData?.Branch || ""}
                                            disabled
                                        />
                                    </div>
                                    <div>
                                        <Input
                                            ...
                                            value={projectWithBankData?.AcType || ""}
                                            disabled
                                        />
                                    </div>
                                </>
