...
export default function WorkHourSummary({ workHourStatus = [], workHourBarGraphStatus = [] }: Props) {
    ...
    console.log("workHourBarGraphStatus", workHourBarGraphStatus);

    const today = new Date();
    const dayName = today.toLocaleString('en-us', { weekday: 'long' });

    // const totalHours = workHourBarGraphStatus.reduce((acc, item) => acc + item.HoursWorked, 0);
    // const totalRequiredHours = workHourBarGraphStatus.reduce((acc, item) => acc + item.RequiredHours, 0);
    ...
    const chartData = workHourBarGraphStatus.map(item => {
        ...
        // Correct minute calculation if it rounds to 60
