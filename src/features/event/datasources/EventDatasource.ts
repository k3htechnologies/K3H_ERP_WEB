...
import { EventsApi } from '@/features/event/api/EventApi'
import type {
...
export class EventDatasourceImpl implements EventDatasource {
    ...
    async pullEvent(params: FilterWithPaginationEventRequest, signal?: AbortSignal): Promise<EventListResponse> {
        try {
            ...
            if (params.Title) queryParams.append('Title', params.Title.trim());
            ...
            const response = await this.k3hHttpClient.getRequestWithAuthentication(
                `${EventsApi.PULL}?${queryParams.toString()}`, { signal }
            )
            return response;
        } catch (error: any) {
    ...
    async deleteEvent(params: DeleteEventRequest): Promise<EventDeleteResponse> {
        try {
            ...
            const response = await this.k3hHttpClient.deleteRequestWithAuthentication(
                `${EventsApi.DELETE}?${queryParams.toString()}`
            )
