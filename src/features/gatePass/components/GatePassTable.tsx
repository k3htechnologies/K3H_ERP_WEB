...
export const GatePassTable: React.FC<GetPassTableProps> = ({
    ...
    const tableColumns = useMemo<TableColumn[]>(() => {
        return columns.map(col => {
            ...
            if (col.key === 'PassDateTime' && col.label === 'Pass Date') {
                return {
                    ...col,
                    render: (_value: any, row: GatePassData) => (
                        <span>{formatDate_dd_MonthName_yy(row.PassDateTime)}</span>
                    ),
                };
            }

            if (col.key === 'PassDateTime' && col.label === 'Pass Time') {
                return {
                    ...col,
                    render: (_value: any, row: GatePassData) => (
                        <span>{parseTimeFromISO(row.PassDateTime)}</span>
                    ),
                };
            }
