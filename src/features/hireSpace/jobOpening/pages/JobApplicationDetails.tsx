...
import { CandidateDetailsPanel, CandidateListPanel, CandidateStageSidebar } from "@/features/hireSpace/jobOpening/components";
import type {
...
import {
  ...
  STATUS_TO_API_VALUE,
  STATUS_TO_STAGE_ID,
} from "@/features/hireSpace/jobOpening/utils/candidateApplication";
    ...
    await runApiWithLoader(
      ...
      async () => {
        ...
        if (E.isRight(response)) {
          const getRemarkDateTime = (remark: CandidateRemarkData) => {
            const date = new Date(remark.ModifiedDate || remark.CreatedDate);
          ...
          const loadedRemarks = (response.right.Data ?? [])
            ...
            .sort((first, second) => {
              return getRemarkDateTime(second) - getRemarkDateTime(first);
            });
