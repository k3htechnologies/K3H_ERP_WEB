...
import type { FilterWithPaginationHolidayMappingMasterRequest, AddUpdateHolidayMappingMasterRequest, DeleteHolidayMappingMasterRequest, HolidayMappingMasterListResponse, HolidayMappingMasterSaveResponse, HolidayMappingMasterDeleteResponse } from "@/features/holidayMappingMaster/models/HolidayMappingMasterModel";
...
export class HolidayMappingMasterDatasourceImpl implements HolidayMappingMasterDatasource {
  ...
  async addUpdateHolidayMappingMaster(params: AddUpdateHolidayMappingMasterRequest): Promise<HolidayMappingMasterSaveResponse> {
    try {
      const response = await this.k3hHttpClient.postRequestWithAuthentication(HolidayMappingMasterApi.ADD_UPDATE, params);
  ...
  async deleteHolidayMappingMaster(params: DeleteHolidayMappingMasterRequest): Promise<HolidayMappingMasterDeleteResponse> {
    try {
      ...
      const response = await this.k3hHttpClient.deleteRequestWithAuthentication(`${HolidayMappingMasterApi.DELETE}?${queryParams.toString()}`);
