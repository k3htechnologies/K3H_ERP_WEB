...
export const LitigationDocument: React.FC = () => {
    ...
    const handleExportLitigationDocuments = async (exportType: 'Excel' | 'PDF') => {
    ...
    const handleExportLitigationDocumentExcel = () => handleExportLitigationDocuments('Excel')
    const handleExportLitigationDocumentPdf = () => handleExportLitigationDocuments('PDF')
    //#endregion

    //#region HANDLE PAGE CHNAGE EVENT
    const handlePageChange = (page: number) => {
        fetchLitigationDocumentList(page);
    };
    //#endregion
    ...
    const LitigationDocumentListForTable = useMemo(() => LitigationDocumentList, [LitigationDocumentList]);
    //#endregion

    //#region EDIT LITIGATION DOCUMENT
    const handleEditLitigationDocument = useCallback((row: LitigationDocumentData) => {
    ...
    const handleAddUpdateLitigationDocument = async (e: React.FormEvent) => {
        ...
        await runApiWithLoader(
            ...
            async () => {
                ...
                const response = await litigationDocumentService.apiCallAddUpdateLitigationDocument(payload);
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          ...
          COMBINED SEARCH BAR, FILTER IMPORT , EXPORT ROW
           ============================================================================ */}
            ...
            {/* FILTER LITIGATION DOCUMENT MODAL */}

            <Modal
                isOpen={showFilterPopup}
                onClose={() => setShowFilterPopup(false)}
                title="Filter - Litigation Document"
                onSubmit={(e) => {
                    e.preventDefault()
                    applyFilters()
                }}
