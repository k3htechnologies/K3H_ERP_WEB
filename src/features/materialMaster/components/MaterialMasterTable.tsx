...
interface MaterialMasterTableProps {
  ...
  onView: (row: MaterialMasterData) => void;
  onEdit: (row: MaterialMasterData) => void;
  onDelete: (row: MaterialMasterData) => void;
  canAction: boolean;
  loading: boolean;
}

export const MaterialMasterTable: React.FC<MaterialMasterTableProps> = ({
  data,
  columns,
  pagination,
  sortInfo,
  ...
  const tableColumns = useMemo<TableColumn[]>(() => {
    return columns.map(col => {
      if (col.key === 'Actions') {
        return {
          ...col,
          render: (_value, row: MaterialMasterData) => (
            canAction ? (
