...
import { fetchBuildingDropdown, fetchWingDropdown } from "../BuildingWingDropdown";
...
export const PaymentScheduleMaster: React.FC = () => {
  ...
  const handleWingChange = async (wing: string) => {
    ...
    await loadPaymentScheduleMaster(1, {
      BuildingId: String(formData.BuildingId),
      Wing: wing
    });
  };
  //#endregion
        ...
        if (E.isRight(response)) {
          ...
          if (isAdd) {
            ...
            await loadPaymentScheduleMaster(
              ...
              { Wing: formData.Wing ?? '', BuildingId: formData.BuildingId ? String(formData.BuildingId) : "" },
            );
            addToast({ type: 'success', title: response.right.SuccessMessage[0] })
          }
    ...
    await runApiWithLoader(
      ...
      async () => {
        ...
        if (E.isRight(response)) {
          ...
          await loadPaymentScheduleMaster(
            pageToShow, {
            BuildingId: formData.BuildingId ? String(formData.BuildingId) : "",
            Wing: formData.Wing ?? ""
          },
          );
          addToast({ type: 'success', title: response.right.SuccessMessage?.[0] })
  ...
  }, [navigate, ratePerSqFt, formData.BuildingId, formData.Wing, addToast]);
  //#endregion
    ...
    <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
      ...
      <div className="space-y-4 pb-5">
        ...
        <div className="flex items-center justify-between border-b border-gray-300 pb-2">
          ...
          {totalPercentage < 100 && formData.BuildingId > 0 && formData.Wing && (
            <Button
              onClick={() => {
                setEditingPaymentScheduleMasterData(null);
                setFormData(prev => ({
      ...
      {/*ADD UPDATE PAYMENT SCHEDULE MODAL */}
      ...
      <Modal
        ...
        saveText={editingPaymentScheduleMasterData ? 'Update Payment Schedule' : 'Add Payment Schedule'}
        loading={isLoading}
        size="xl"
      >
