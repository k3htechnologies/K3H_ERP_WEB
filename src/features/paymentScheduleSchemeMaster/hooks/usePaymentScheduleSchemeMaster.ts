...
import {
    ...
    fetchWingDropdown,
} from "@/features/inventory/InventoryDropdown";
...
export const usePaymentScheduleSchemeMaster = () => {
    ...
    const debouncedSearch = useDebouncedCallback((value: string) => {searchPaymentScheduleSchemeMaster(value);}, 350);
    ...
    //Initialization - Fetching Payment Schedule Scheme Master List
    useEffect(() => {
    ...
    useEffect(() => {
        setSelectedPaymentScheduleSchemeMasterColumnKeys((prev) =>
            Array.from(
                new Set([...prev, ...requiredPaymentScheduleSchemeMasterColumnKeys]),
            ).filter((k) => allPaymentScheduleSchemeMasterColumnKeys.includes(k)),
        );
    }, []);
    ...
    const paymentScheduleSchemeMasterColumns = useMemo<TableColumn[]>(
        () => getPaymentScheduleSchemeMasterColumns(),
        [],
    );
    //#endregion
    ...
    const handleFieldChange = (
        field: keyof AddUpdatePaymentScheduleSchemeMasterRequest,
        value: any,
    ) => {
    ...
    const handleAddEditPaymentScheduleSchemeMaster = async (
        e: React.FormEvent,
    ) => {
        e.preventDefault();
        setErrors({});
        ...
        await runApiWithLoader(
            ...
            async () => {
                ...
                const response =
                    await paymentScheduleSchemeMasterService.apiCallAddUpdatePaymentScheduleSchemeMaster(
                        payload,
                    );
                ...
                if (E.isRight(response)) {
                    ...
                    const isAdd =
                        ...
                        !formData.PaymentScheduleSchemeMasterId;

                    if (isAdd) {
                        const newRecord = response.right
                            .Data[0] as PaymentScheduleSchemeMasterData;
                        ...
                        setPagination({
                            ...
                            totalPages: Math.ceil(
                                (pagination.totalRecords + 1) / pagination.pageSize,
                            ),
                        });
                        ...
                        const updatedRecord = response.right
                            .Data[0] as PaymentScheduleSchemeMasterData;

                        setPaymentScheduleSchemeMasterList((prevData) =>
                            prevData.map((item) =>
                                item.PaymentScheduleSchemeMasterId ===
                                    formData.PaymentScheduleSchemeMasterId
                                    ? updatedRecord
                                    : item,
                            ),
    ...
    const fetchPaymentScheduleSchemeMasterList = async (
        page: number = pagination.currentPage,
        sort?: SortInfo,
    ) => {
        return await loadPaymentScheduleSchemeMaster(
            page,
            filters,
            sort ?? sortInfo,
        );
                ...
                async () => {
                    const params: FilterWithPaginationPaymentScheduleSchemeMaster = {
                        ...
                        PaymentScheduleSchemeMasterId:
                            filterParams.PaymentScheduleSchemeMasterId
                                ? Number(filterParams.PaymentScheduleSchemeMasterId)
                                : 0,
                        PaymentScheduleScheme:
                            searchtext ?? filterParams.PaymentScheduleScheme ?? undefined,
    ...
    const requiredPaymentScheduleSchemeMasterColumnKeys: string[] =
        REQUIRED_COLUMN_KEYS;

    const allPaymentScheduleSchemeMasterColumnKeys: string[] =
        paymentScheduleSchemeMasterColumns.map((c) => c.key);
    ...
    const [
        ...
        try {
            const saved =
                LocalStorageHelper.getPaymentScheduleSchemeMasterTableColumns();
            if (saved) {
    ...
    const visiblePaymentScheduleSchemeMasterColumns = useMemo(
        () =>
            paymentScheduleSchemeMasterColumns.filter((col) =>
                selectedPaymentScheduleSchemeMasterColumnKeys.includes(col.key),
            ),
        [
            paymentScheduleSchemeMasterColumns,
            selectedPaymentScheduleSchemeMasterColumnKeys,
    ...
    //#region VIEW EDIT
    const handleViewPaymentScheduleSchemeMasterDetails = useCallback(
        (row: PaymentScheduleSchemeMasterData) => {
    ...
    const handleEditPaymentScheduleSchemeMasterDetails = useCallback(
        (row: PaymentScheduleSchemeMasterData) => {
            ...
            setFormData({
                ...
                InventoryFlatFloorBasementPodiumWingId:
                    row.InventoryFlatFloorBasementPodiumWingId || 0,
                Wing: row.Wing || "",
            });
    ...
    const handleExportPaymentScheduleSchemeMaster = async (
        ...
        await runApiWithLoader(
            ...
            async () => {
                const params: FilterWithPaginationPaymentScheduleSchemeMaster = {
                    ...
                    ProjectId: Number(projectId),
                    PaymentScheduleScheme:
                        filters.PaymentScheduleScheme?.trim() || undefined,
                    SortBy: getSortByParam(
                        sortInfo ?? null,
                        paymentScheduleSchemeMasterColumns,
                    ),
    ...
    const handleExportPaymentScheduleSchemeMasterExcel = () =>
        handleExportPaymentScheduleSchemeMaster("Excel");
    const handleExportPaymentScheduleSchemeMasterPdf = () =>
        handleExportPaymentScheduleSchemeMaster("PDF");
    //#endregion
    ...
    const handleSortColumn = useCallback(
        (sort: SortInfo) => {
            setSortInfo(sort);
            loadPaymentScheduleSchemeMaster(
                1,
                filters,
                sort,
    ...
    await runApiWithLoader(
      ...
      async () => {
        ...
        const params: DeletePaymentScheduleSchemeMasterRequest = {
                    PaymentScheduleSchemeMasterId:
                        deletePaymentScheduleSchemeMasterDetailsData.PaymentScheduleSchemeMasterId,
                    ProjectId: Number(projectId),
