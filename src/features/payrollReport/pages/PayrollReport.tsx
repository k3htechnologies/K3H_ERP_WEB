...
import { DataTable, type PaginationInfo, type SortInfo } from '@/ui/components/DataTable/DataTable';
...
export const PayrollReport: React.FC = () => {
  ...
  // ── Debounced search ───────────────────────────────────────────────────────
  const debouncedSearch = useDebouncedCallback((_v: string) => dispatchLoad(1), 350);

  // ── Pagination & sorting ───────────────────────────────────────────────────
  const handlePageChange = useCallback((page: number) => dispatchLoad(page), [dispatchLoad]);
  ...
  // ── Shared table props (avoids repeating on every table instance) ──────────
    ...
    <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
      ...
      <div className="mt-4">
        <TableActionToolbar
          ...
          onSearchChange={v => { setSearchTerm(v); debouncedSearch(v); }}
          onClearSearch={clearSearch}
          isShowFilterButton
          filters={filters}
          onOpenFilter={() => setShowFilterPopup(true)}
          isShowExportButton
          onExportPdf={() => handleExportPdf(() => getCurrentColumns(activeTab), sortInfo)}
      ...
      >
        <div className="space-y-6">
          <div className="space-y-4">
            {/* Resignation uses different field names; all other tabs share Start/End Date */}
              ...
              <>
                <div>
                  <DatePickerInput
                    ...
                    onChange={(v) => handleFilterChange('ResignationDateFrom', v || '')}
                  />
                </div>
                <div>
                  <DatePickerInput
                    ...
                    onChange={(v) => handleFilterChange('ResignationDateTo', v || '')}
                  />
                </div>
              </>
            ) : (
