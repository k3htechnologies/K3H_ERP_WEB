...
const ProcurementMaster: React.FC<Props> = ({ procurementMasterData = [] }: Props) => {
  ...
  return (
    <div className="pt-5 flex flex-col h-full">
      ...
      <div className="w-full bg-white py-5 border border-gray-100 flex flex-col sm:flex-row divide-y sm:divide-y-0 sm:divide-x divide-gray-200 rounded-xl flex-1 shadow-sm">
        {data.map((c, i) => {
          return (
      ...
      <div className="mt-5">
        <div className="w-full bg-yellow-50 p-3 border border-yellow-200  rounded-md flex items-center justify-between shadow-sm">
          <div>
