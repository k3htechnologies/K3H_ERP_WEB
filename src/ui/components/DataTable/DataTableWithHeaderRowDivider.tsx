...
interface DataTableWithHeaderRowDividerProps {
    data: any[]
    columns: TableColumn[]
...
export const DataTableWithHeaderRowDivider: React.FC<DataTableWithHeaderRowDividerProps> = ({
    data,
    columns,
    pagination,
    loading = false,
        ...
        <div className={`rounded-lg flex flex-col}  ${fixedHeight ? 'h-full' : ''} ${className}`}>
            ...
            <div ref={scrollRef}  className={`overflow-x-auto thin-scroll ${fixedHeight ? 'flex-1 overflow-y-auto' : ''}`} style={fixedHeight ? {
                ...
                <table className={`min-w-full border-collapse`}>
                    ...
                    >
                        <tr
                            className="h-10 border-b border-[#E5E7EB]"
                            style={{
                                fontSize: '14px',
                                fontWeight: '500',
                                lineHeight: '1.4',
                        ...
                        >
                            {columns.map((column) => (
                                ...
                                >
                                    ...
                                    >
                                        <span className="truncate text-[#1D1D1D80]">{column.label}</span>
                                        {column.sortable && (
                    ...
                    <tbody>
                        {!loading && data.length === 0 ? (
                            ...
                            (
                                data.map((row, index) => (
                                    <tr key={index} className="h-10 border-b border-[#E5E7EB] hover:bg-gray-50">
                                        {columns.map((column) => {
                                            ...
                                            return (
                                                <td
                                                    key={column.key}
                                                    className={`px-4 py-2 text-gray-900 ${column.align === 'center' ? 'text-center' :
                                                        column.align === 'right' ? 'text-right' : 'text-left'
                                                        } ${column.fixed === 'left' ? 'sticky left-0 bg-white z-20 shadow-[2px_0_4px_rgba(0,0,0,0.1)]' :
